...
func main() {
	var (
		...
		authFile  = flag.String("auth", auth.AuthFile, "Path to auth credential file")
	...
	if *initAuth {
		if err := auth.InitAuthInteractive(*authFile); err != nil {
	...
	if *srvMode {
		cred, err := auth.LoadCredential(*authFile)
		if err != nil {
		...
		srv := server.New(*addr, pager, schema, cred)
